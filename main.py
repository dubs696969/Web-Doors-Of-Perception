...
# --- Touch Controls Data ---
touch_controls = []
is_touch_up_pressed = False
is_touch_down_pressed = False
is_touch_left_pressed = False
is_touch_right_pressed = False
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, keys, walls: pygame.sprite.Group, coins: pygame.sprite.Group, current_game_state_setter):
        ...
        if keys[pygame.K_LEFT] or is_touch_left_pressed:
            self.rect.x -= self.speed
        elif keys[pygame.K_RIGHT] or is_touch_right_pressed:
            self.rect.x += self.speed
        ...
        if keys[pygame.K_UP] or is_touch_up_pressed:
            self.rect.y -= self.speed
        elif keys[pygame.K_DOWN] or is_touch_down_pressed:
            self.rect.y += self.speed
...
def initialise_game_objects():
    ...
    global touch_controls # Access the global touch_controls list
    global is_touch_up_pressed, is_touch_down_pressed, is_touch_left_pressed, is_touch_right_pressed
    ...
    # Define screen dimensions for button positioning
    ...
    touch_controls = [
        {"name": "UP", "rect": pygame.Rect(85, screen_height - 190, 60, 60), "color": button_color, "label": None},
        {"name": "DOWN", "rect": pygame.Rect(85, screen_height - 50, 60, 60), "color": button_color, "label": None},
        {"name": "LEFT", "rect": pygame.Rect(50, screen_height - 120, 60, 60), "color": button_color, "label": None},
        {"name": "RIGHT", "rect": pygame.Rect(120, screen_height - 120, 60, 60), "color": button_color, "label": None},
        {"name": "PORTAL", "rect": pygame.Rect(screen_width - 130, screen_height - 100, 80, 60), "color": button_color, "label": "P"},
    ]
...
def draw_touch_controls(surface, controls):
    """Draws the touch controls."""
    for control in controls:
        pygame.draw.rect(surface, control["color"], control["rect"])
        if control["label"]:
            label_surface = my_font.render(control["label"], 1, (255, 255, 255)) # White text
            label_rect = label_surface.get_rect(center=control["rect"].center)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if current_game_state == GAME_STATE_PLAYING:
                    for button_def in touch_controls:
                        if button_def['rect'].collidepoint(event.pos):
                            if button_def['name'] == "UP":
                                is_touch_up_pressed = True
                            elif button_def['name'] == "DOWN":
                                is_touch_down_pressed = True
                            elif button_def['name'] == "LEFT":
                                is_touch_left_pressed = True
                            elif button_def['name'] == "RIGHT":
                                is_touch_right_pressed = True
                            elif button_def['name'] == "PORTAL":
            ...
            if current_game_state == GAME_STATE_PLAYING:
                 ...
                 if current_game_state == GAME_STATE_PLAYING:
                    ...
                    if current_game_state == GAME_STATE_PLAYING:
                        ...
                        # Draw touch controls
                        draw_touch_controls(screen, touch_controls)
